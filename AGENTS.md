...
- When a generated sidecar route receives `*scope.NotFoundError` from Runtime
  Scope admission, delegate it to the shared endpoint error mapper instead of
  converting it to an internal error. Verify every affected route returns 404
  `scope_not_found`, redacts protected values, and creates no durable side effects.
